import pandas as pd
import repository.partnerRepository as partnerRepository

# => Obtem documentos de uma coleção
def get_docs():    
    df = pd.DataFrame()    
    docs = partnerRepository.get_all()
    for doc in docs:
...
def delete_doc(field, value):
    doc = partnerRepository.find(field, value)
    if doc:
        try:        
            partnerRepository.delete(doc)
            response = {
                "status": "success",
...
def update(field, value, new_doc):
    ...
    if not valido:
        ...
        if doc:
            try:
                partnerRepository.update(doc, new_doc)
                response = {
                    "status": "success",
...
def validaUnique(value):
    ...
    doc = partnerRepository.find("nick_name", value)         # <=
    if doc:
        status = False
